...
import json
from util import insert_new_line, get_article_date, download_image, download_video, get_valid_filename


def read_cookies_from_file(cookies_path):
    """
    从文件读取 Cookie，兼容直接复制 Cookie header 的写法。
    """
...
class ZhihuParser:
    ...
    def check_connect_error(self, target_link):
        ...
        try:
            ...
            response = self.session.get(target_link, headers=headers, timeout=30)
            if response.status_code == 403:
                if "zh-zse-ck" in response.text or "zse_ck" in response.text:
                    raise ValueError(
                        "Zhihu returned 403 browser verification page (zse_ck). "
                        "Please open the target URL in a logged-in browser, then copy the latest request Cookie header to cookies.txt. "
                        "Make sure it includes valid z_c0 and __zse_ck values."
                    )
                raise ValueError(
                    "Zhihu returned 403 Forbidden. Please refresh cookies.txt with valid logged-in Zhihu cookies."
                )
...
if __name__ == "__main__":
    ...
    # 回答
    url = "https://www.zhihu.com/question/498271491/answer/2030554766640673023"
    ...
    markdown_path = f"{markdown_title}.md"
    if os.path.isfile(markdown_path):
        print(f"Saved markdown result to: {markdown_path}")
    else:
        print(f"Saved markdown result to: {markdown_title}")
